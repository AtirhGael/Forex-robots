...
static input int    Magic_Number = 1636834902; // Magic number
static input double Entry_Amount =     0.01; // Entry lots
       input int    Stop_Loss    =       65; // Stop Loss   (pips)
       input int    Take_Profit  =       98; // Take Profit (pips)

static input string ___0______   = "------"; // --- DeMarker ---
       input int    Ind0Param0   =       14; // Period
       input double Ind0Param1   =     0.00; // Level

static input string ___1______   = "------"; // --- Stochastic Signal ---
       input int    Ind1Param0   =       16; // %K Period
       input int    Ind1Param1   =       14; // %D Period
       input int    Ind1Param2   =       15; // Slowing

static input string ___2______   = "------"; // --- Bulls Power ---
       input int    Ind2Param0   =        7; // Period
       input double Ind2Param1   =   0.0000; // Level

static input string ___3______   = "------"; // --- Candle Color ---
       input int    Ind3Param0   =       38; // Min body height [pips]
       input int    Ind3Param1   =        1; // Consecutive candles
...
static input string News_Currencies   = "USD,JPY"; // News currencies
...
int GetEntrySignal(void)
  {
   // DeMarker (14)
   double ind0val1  = iDeMarker(NULL, 0, Ind0Param0, 1);
   double ind0val2  = iDeMarker(NULL, 0, Ind0Param0, 2);
   double ind0val3  = iDeMarker(NULL, 0, Ind0Param0, 3);
   ...
   double ind2val1  = iBullsPower(NULL, 0, Ind2Param0, PRICE_CLOSE, 1);
...
void ManageClose(void)
  {
   ...
   bool ind3short = false;
   {
      ...
      double pipVal = pip * Ind3Param0;

      for (int b = Ind3Param1 + 2; b > 0; b--)
        {
      ...
      ind3long  = consecutiveBearish >= Ind3Param1;
      ind3short = consecutiveBullish >= Ind3Param1;
   }
...
//+------------------------------------------------------------------+
/*STRATEGY MARKET BlackBull; USDJPY; H1 */
/*STRATEGY CODE {"properties":{"entryLots":0.1,"tradeDirectionMode":0,"oppositeEntrySignal":0,"stopLoss":65,"takeProfit":98,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"DeMarker","listIndexes":[6,0,0,0,0],"numValues":[14,0,0,0,0,0]},{"name":"Stochastic Signal","listIndexes":[2,0,0,0,0],"numValues":[16,14,15,0,0,0]},{"name":"Bulls Power","listIndexes":[2,0,0,0,0],"numValues":[7,0,0,0,0,0]}],"closeFilters":[{"name":"Candle Color","listIndexes":[1,0,0,0,0],"numValues":[38,1,0,0,0,0]}]} */
